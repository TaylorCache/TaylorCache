<script>
    ...
    let videoGroups = document.querySelectorAll(".video-group");
    let totalGroups = videoGroups.length;
    ...
    currentGroupIndex = (currentGroupIndex + 1) % totalGroups; // 循环到下一个组
    showGroup(currentGroupIndex);
});

    document.getElementById("prev-video").addEventListener("click", function() {
    currentGroupIndex = (currentGroupIndex - 1 + totalGroups) % totalGroups; // 循环到上一个组
    showGroup(currentGroupIndex);
});

    showGroup(currentGroupIndex); // 初始显示第一个组
});
</script>
